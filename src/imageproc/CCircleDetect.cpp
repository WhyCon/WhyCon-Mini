...
void CCircleDetect::identifySegment(SSegment* segment)
{
	...
	segment->ID = -10000;
	if (calibMode == false){
		if (segment->m0/segment->m1 > 0.1){
			x = (segment->r0*160-40)/15;
			y = (segment->r1*130-40)/15;
			int ix = round(x);
			int iy = round(y);
			x -= ix;
			y -= iy;
			if (debug > 5) printf("ID: %i %.3f\n",ix*5+iy,sqrt(x*x+y*y));
			segment->ID = ix*4+iy;
		}else{
			if (debug > 2) printf("Ratio: %.3f - calibration pattern.\n",segment->m0/segment->m1);
		}
	}else{
		if (segment->m0/segment->m1 < 1.1){
			if (segment->r0 < segment->r1){
				x = segment->r0;
				segment->r0 = segment->r1;
				segment->r1 = x;
			}
			x = (segment->r0*150-40)/15;
			y = (segment->r1*150-40)/30;
			int ix = round(x);
			int iy = round(y);
			x -= ix;
			y -= iy;
			if (debug > 5) printf("ID: %i %i %.3f %.3f\n",ix,iy,x,y);
			segment->ID = ix+iy;
		}else{
			if (debug > 2) printf("Ratio: %.3f - not a calibration pattern.\n",segment->m0/segment->m1);
		}
	}
}
